...
server.setRequestHandler(CallToolRequestSchema, async (request: CallToolRequest) => {
  ...
  if (name === "get_push_count") {
    try {
      ...
      // Для почасового отчета фильтруем события за последний час
      if (days === 1) {
        const now = new Date();
        const endTime = new Date(now);
        endTime.setMinutes(0, 0, 0); // Округляем до часа
        const startTime = new Date(endTime);
        startTime.setHours(endTime.getHours() - 1); // Минус один час
        
        pushEvents = pushEvents.filter((event: any) => {
      ...
      // Определяем формат в зависимости от периода
      let resultText: string;
      if (days === 1) {
        // Почасовой формат - за последний час
        const now = new Date();
        const endTime = new Date(now);
        endTime.setMinutes(0, 0, 0); // Округляем до часа
        const startTime = new Date(endTime);
        ...
        const endHourStr = endTime.getHours().toString().padStart(2, '0');
        
        resultText = `D09\n\n`;
        resultText += `📊 Статистика пушей для пользователя **${username}** за последний час:\n\n`;
        resultText += `🔢 **Количество пушей:** ${pushCount}\n`;
        resultText += `📅 **Период:** с ${startDateStr} ${startHourStr}:00 по ${endDateStr} ${endHourStr}:00`;
      } else {
        // Долгосрочный формат
        resultText = `D09\n\n`;
        resultText += `📊 Статистика пушей для пользователя **${username}** за последние ${days} дней:\n\n`;
        resultText += `🔢 **Количество пушей:** ${pushCount}\n`;
