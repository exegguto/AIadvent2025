            ...
            Column(
                ...
                val info = p?.let {
                    buildString {
                        ...
                        // Show MCP/Service source and phase if present
                        val via = it.metadata?.get("via")
                        val source = it.metadata?.get("source")
                        val phase = it.metadata?.get("phase")
                        if (!via.isNullOrBlank() || !source.isNullOrBlank() || !phase.isNullOrBlank()) {
                            if (parts.isNotEmpty()) parts.add(0, "·")
                            val tagParts = mutableListOf<String>()
                            if (!source.isNullOrBlank()) tagParts.add(source)
                            if (!via.isNullOrBlank()) tagParts.add(via)
                            if (!phase.isNullOrBlank()) tagParts.add(phase)
                            parts.add(tagParts.joinToString("/"))
                        }
                        if (parts.isNotEmpty()) {
                            append("  •  ")
