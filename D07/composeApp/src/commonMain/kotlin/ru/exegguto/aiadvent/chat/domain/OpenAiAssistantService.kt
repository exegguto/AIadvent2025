    ...
    override suspend fun sendMessage(
        ...
        val history = buildList {
            add(
                ChatMessage(
                    "system",
                    "Отвечай строго в формате JSON без markdown и лишнего текста. Схема ответа: {\"answer\": string, \"points\": string[], \"summary\": string, \"mcp\"?: {\"tool\": \"github\", \"action\": \"list\", \"target\": \"repos|issues|pulls\", \"owner\"?: string, \"repo\"?: string}}. Пример: {\"answer\": \"краткий ответ\", \"points\":[\"шаг 1\",\"шаг 2\"], \"summary\": \"итог\"}. Если тебе нужны данные GitHub — верни поле mcp с запросом. Если данных не хватает — всё равно верни валидный JSON. Ты имеешь доступ к MCP-инструментам. Используй их только когда нужны внешние данные. После получения данных если необходимо, вызывай MCP повторно с нужной тебе ручкой."
                )
            )
